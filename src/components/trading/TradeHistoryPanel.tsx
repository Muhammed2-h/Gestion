  ...
  return (
    <div className="flex flex-col h-full">
      <div className="pb-3 mb-2 border-b border-border flex items-center justify-between">
        ...
        <span className={`text-[0.75rem] font-mono font-bold ${totalRealizedPnl >= 0 ? 'text-profit' : 'text-loss'}`}>
          ...
          <tbody>
            {tradeHistory.map(t => (
              <tr key={t.id} className="border-b border-border/50 last:border-0 hover:bg-bg-primary transition-fast">
                <td className="text-[0.65rem] text-muted py-2">
                ...
                <td className="py-2">
                  <Badge variant={t.side === 'buy' ? 'profit' : 'loss'} className="text-[0.6rem] py-0 px-1.5 uppercase">
                    {t.side}
                  </Badge>
                </td>
